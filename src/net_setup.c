...
static bool setup_myself(void) {
	...
	/* Check if VLESS mode is enabled - if so, we don't need legacy keys */
	bool vless_mode = false;
	if(get_config_bool(lookup_config(config_tree, "VLESSMode"), &vless_mode) && vless_mode) {
		logger(DEBUG_ALWAYS, LOG_INFO, "VLESS mode enabled, legacy keys optional.");
	}
...
bool setup_network(void) {
	...
	/* Initialize hosts.json database for VLESS */
	if(!hosts_db_init(confbase)) {
		logger(DEBUG_ALWAYS, LOG_WARNING, "Failed to initialize hosts.json database");
	} else {
		hosts_db_dump();
	}
	...
	if(vless_mode) {
		logger(DEBUG_ALWAYS, LOG_INFO, "VLESS mode enabled");

		/* Read VLESS UUID */
		get_config_string(lookup_config(config_tree, "VLESSUUID"), &vless_uuid);

		if(!vless_uuid) {
			logger(DEBUG_ALWAYS, LOG_WARNING, "VLESSMode enabled but VLESSUUID not configured, generating new UUID");
			/* Generate UUID will be done in net_socket.c */
		}

		/* Read Reality configuration */
		get_config_bool(lookup_config(config_tree, "VLESSReality"), &vless_reality_enabled);

		if(vless_reality_enabled) {
			logger(DEBUG_ALWAYS, LOG_INFO, "VLESS Reality protocol enabled");

			/* Reality destination (fallback) */
			get_config_string(lookup_config(config_tree, "VLESSRealityDest"), &vless_reality_dest);

			if(!vless_reality_dest) {
				logger(DEBUG_ALWAYS, LOG_ERR, "VLESSReality enabled but VLESSRealityDest not configured");
				return false;
			}

			if(!get_config_int(lookup_config(config_tree, "VLESSRealityDestPort"), &vless_reality_dest_port)) {
				vless_reality_dest_port = 443;
			}

			/* Reality server name (SNI) */
			get_config_string(lookup_config(config_tree, "VLESSRealityServerName"), &vless_reality_server_name);

			/* Reality keys */
			get_config_string(lookup_config(config_tree, "VLESSRealityPublicKey"), &vless_reality_public_key);
			get_config_string(lookup_config(config_tree, "VLESSRealityPrivateKey"), &vless_reality_private_key);

			if(!vless_reality_private_key) {
				logger(DEBUG_ALWAYS, LOG_ERR, "VLESSReality enabled but VLESSRealityPrivateKey not configured");
				return false;
			}

			/* Reality ShortID */
			get_config_string(lookup_config(config_tree, "VLESSRealityShortID"), &vless_reality_short_id);

			/* Reality fingerprint */
			get_config_string(lookup_config(config_tree, "VLESSRealityFingerprint"), &vless_reality_fingerprint);

			if(!vless_reality_fingerprint) {
				vless_reality_fingerprint = xstrdup("chrome");
			}

			logger(DEBUG_ALWAYS, LOG_INFO, "Reality configured: dest=%s:%d, sni=%s, fingerprint=%s",
			       vless_reality_dest, vless_reality_dest_port,
			       vless_reality_server_name ? vless_reality_server_name : "(auto)",
	...
	/* Load VPNAddress and Route configuration before enabling device */
